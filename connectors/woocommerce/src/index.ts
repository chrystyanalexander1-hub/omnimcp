...
import { z } from "zod";
import { wooRequest } from "./woocommerce-client.js";
  ...
  tools: [
    {
      ...
      async handler({ storeUrl, search, perPage }) {
        return jsonResult(await wooRequest<unknown[]>(storeUrl, "products", { per_page: perPage, ...(search ? { search } : {}) }));
      },
    },
    {
      name: "list_orders",
      ...
      async handler({ storeUrl, status, perPage }) {
        return jsonResult(await wooRequest<unknown[]>(storeUrl, "orders", { per_page: perPage, ...(status ? { status } : {}) }));
      },
    },
    {
      ...
      async handler({ storeUrl, orderId, status }) {
        await wooRequest(storeUrl, `orders/${orderId}`, { status }, "PUT");
        return textResult(`Order ${orderId} status set to ${status}`);
      },
    },
  ],
});
