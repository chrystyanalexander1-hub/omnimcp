  ...
  tools: [
    {
      ...
      async handler() {
        ...
        const { accounts } = await handle<{ accounts: unknown[] }>(res);
    ...
    },
    {
      ...
      async handler({ baseUri, accountId, fromDate }) {
        ...
        const { envelopes } = await handle<{ envelopes?: unknown[] }>(res);
    ...
    },
    {
      ...
      async handler({ baseUri, accountId, envelopeId }) {
        ...
        return jsonResult(await handle<{ status: string }>(res));
      },
    },
    {
      /**
      ...
      async handler({ baseUri, accountId, emailSubject, documentBase64, documentName, fileExtension, signerEmail, signerName }) {
        ...
        const { envelopeId } = await handle<{ envelopeId: string }>(res);
        return jsonResult({ envelopeId });
      },
    },
  ],
});
